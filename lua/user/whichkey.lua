...
local normal = {
  ...
  g = {
    ...
    ["."]     = map("Code actions",               "<cmd>lua vim.lsp.buf.code_action()<CR>"),
  },
  ...
  ["<leader>"] = {
    ...
    l = {
      ...
      a       = map("Code actions",               "<cmd>lua vim.lsp.buf.code_action()<CR>"),
      l       = map("CodeLens action",            "<cmd>lua vim.lsp.codelens.run()<CR>"),
      r       = map("Rename",                     "<cmd>lua vim.lsp.buf.rename()<CR>"),
...
which_key.register(operatorPending, { mode = "o", prefix = "", buffer = nil, silent = true, noremap = true, nowait = true, })
